...
*   **Cout** réduit.  
    ...
    En ayant une solution auto hébergé, vous n’avez plus besoin de payer mensuelles des outils car vous n’êtes plus connecté au cloud de l’éditeur. (enfin presque, j’explique cela à la suite de ce post). Le seul coût est celui du serveur VPS ou de votre infrastructure.  
    
*   **Personnalisation**  
      
    Cela vous permet de contrôler le code source et vous permet (Si vous êtes à l’aise) de pourquoi pas rajouter certaines fonctionnalités que vous n’auriez pas pu faire avec le service cloud.  
...
*   **Sécurité** (Le plus important)  
    ...
    Auto-héberger votre infrastructure est par essence plus sécurisée car vous avez la charge de vos propres données tandis qu’avec un service cloud, vous déléguez cette tâche à l’éditeur.  
...
Caddyfile est le fichier de configuration qui permet de gérer le reverse proxy du serveur, et de gérer les urls ou se trouveront tes outils.  
...
Entrez votre adresse emails et Chiffrer le mot de passe que vous souhaitez avec cette commande sur votre terminale.
